...
class PFLocaliser(PFLocaliserBase):
    ...
    def update_particle_cloud(self, scan):
        ...
        # find weights
        cumulative_weights = [0] * (NUM_PARTICLES + NUM_RANDOM_PARTICLES)
        total_weight = 0
        for i in range(NUM_PARTICLES):
        ...
        for i in range(NUM_PARTICLES, NUM_PARTICLES + NUM_RANDOM_PARTICLES):
            total_weight += self.sensor_model.get_weight(
                    scan,
                    self.random_particles[i - NUM_PARTICLES]
                    )
            cumulative_weights[i] = total_weight
