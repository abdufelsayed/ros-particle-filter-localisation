...
def systematic_resampling(cumulative_weights, particlecloud):

    """
    Linear search through the weights at equal intervals
    to sample particles for the new cloud
    
    :Args:
        | cumulative_weights: the cumulative list of weights
        | particlecloud: the list of all poses from the previous cloud
    ...
    for i in range(n):
        while val <= cumulative_weights[i]:
            cloud.poses.append(copy_pose(particlecloud[i - 1]))
            val += interval
...
class PFLocaliser(PFLocaliserBase):
    ...
    def update_particle_cloud(self, scan):
        ...
        new_cloud = PoseArray()
        new_cloud = systematic_resampling(cumulative_weights, self.particlecloud.poses)
